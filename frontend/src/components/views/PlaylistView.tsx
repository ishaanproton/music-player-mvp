...
export const PlaylistView: React.FC<PlaylistViewProps> = ({ playlistId, setCurrentView }) => {
    ...
    return (
        <div className="h-full flex flex-col overflow-y-auto">
            {/* Header / Hero */}
            <div className="bg-gradient-to-b from-purple-700 to-surface p-4 sm:p-8 flex flex-col sm:flex-row items-center sm:items-end space-y-4 sm:space-y-0 sm:space-x-6 relative">
                <div className="w-32 h-32 sm:w-48 sm:h-48 bg-gradient-to-br from-purple-500 to-indigo-300 shadow-2xl flex items-center justify-center rounded-sm">
                    <Music className="w-12 h-12 sm:w-16 sm:h-16 text-white" />
                </div>
                <div className="flex flex-col text-white flex-1 text-center sm:text-left">
                    <span className="text-sm font-bold tracking-wider mb-2">Playlist</span>
                    <h1 className="text-3xl sm:text-6xl font-black mb-4 sm:mb-6 tracking-tighter truncate">{playlist.name}</h1>
                    <span className="text-sm font-medium">{tracks.length} songs</span>
                </div>
                <button 
                    onClick={handleDelete}
                    className="absolute top-4 right-4 sm:top-8 sm:right-8 p-2 sm:p-3 bg-red-500/20 text-red-400 rounded-full hover:bg-red-500 hover:text-white transition-colors"
            ...
            {/* Actions */}
            <div className="px-4 sm:px-8 py-4 sm:py-6 flex items-center space-x-6">
                <button 
