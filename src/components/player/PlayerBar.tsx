...
import { 
    ...
    Volume2, VolumeX, Repeat, Shuffle, Heart, Mic2, ListMusic
} from 'lucide-react';
...
export const PlayerBar = () => {
    const { 
        ...
        setActiveArtistId, showQueue, toggleQueue
    } = usePlayerStore();
    ...
    if (!currentTrack) {
        return (
            <>
                ...
                <div className="h-24 bg-surface border-t border-white/5 items-center justify-center absolute bottom-0 left-0 right-0 z-50 hidden md:flex">
                </div>
            </>
        );
    }
    ...
    return (
        <>
            <div ref={playerRef} className="hidden" />
            <div className="h-24 bg-surface border-t border-white/5 items-center justify-between px-4 sm:px-6 absolute bottom-0 left-0 right-0 z-50 hidden md:flex">
            ...
            <div className="flex items-center justify-end w-1/3 space-x-4">
                <button 
                    onClick={toggleQueue} 
                    className={`${showQueue ? 'text-primary' : 'text-textSecondary'} hover:text-white transition`}
                >
