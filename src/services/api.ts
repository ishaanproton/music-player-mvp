...
export const api = {
    ...
    async getAlbumTracks(albumId: string): Promise<Track[]> {
        const res = await fetch(`${API_BASE}/album?id=${encodeURIComponent(albumId)}`);
    ...
    async getArtistProfile(artistId: string): Promise<import('../types').ArtistProfile> {
        const res = await fetch(`${API_BASE}/artist?id=${encodeURIComponent(artistId)}`);
        if (!res.ok) throw new Error('Failed to fetch artist profile');
        return await res.json();
    },

    async getLyrics(trackId: string): Promise<{text: string, source: string}> {
        const res = await fetch(`${API_BASE}/lyrics?id=${encodeURIComponent(trackId)}`);
    ...
    async getRadio(track: Track): Promise<Track[]> {
        ...
        try {
            const res = await fetch(`${API_BASE}/radio?id=${encodeURIComponent(track.id)}`);
            if (!res.ok) return [];
            const data = await res.json();
